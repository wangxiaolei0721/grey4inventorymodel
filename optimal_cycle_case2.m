...
profit_simu = profit(d,theta,lambda,p,c,h,A,T_eval);
% optimal order cycle corresponding to simulated parameters
T_simu_optimal = optimal_cycle(d,theta,lambda,p,c,h,A,T_interval);
% optimal order quantity corresponding to estimated parameters
Q_simu_optimal=T2Q(d,theta,lambda,T_simu_optimal);
% optimal profit corresponding to simulated parameters
profit_simu_optimal = profit(d,theta,lambda,p,c,h,A,T_simu_optimal);
%
syms T;
profit_syms = profit(d,theta,lambda,p,c,h,A,T);
profit_simu_der1=diff(profit_syms,1);
% T_optimal=vpasolve(profit_der1==0,T,T_interval);
profit_simu_der2=diff(profit_syms,2);
matlabFunction(profit_simu_der1,"File","profit_simu_derivative1");
matlabFunction(profit_simu_der2,"File","profit_simu_derivative2");
profit_simu_der1=profit_simu_derivative1(T_eval);
profit_simu_der2=profit_simu_derivative2(T_eval);
%% profit_appro
% profit corresponding to simulated parameters
...
nexttile
%
plot(T_eval,profit_simu,'LineStyle','-','LineWidth',1.5)
hold on;
...
%
plot(T_simu_optimal,profit_simu_optimal,'LineStyle','none','Marker','*','MarkerSize',8,'Color',[0, 114, 189]/255)
...
nexttile
plot(T_eval,profit_simu_der1,'LineStyle','-','LineWidth',1.5)
hold on
...
nexttile
plot(T_eval,profit_simu_der2,'LineStyle','-','LineWidth',1.5)
hold on
...
exportgraphics(gcf,'.\figure\case2_optimization.pdf')
%exportgraphics(gcf,'F:\博士\Grey4inventorydynamics\manuscript4\figure\case2_optimization.pdf')
